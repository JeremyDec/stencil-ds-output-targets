...
import { APPLY_POLYFILLS, BASIC_TYPES, DEFINE_CUSTOM_ELEMENTS } from './constants';
import { OutputTargetAngularJS } from './types';

/**
 * Get components a part of excluded ones.
 ...
 */
function getLoaderImports(config: Config, outputTarget: OutputTargetAngularJS): string {
  ...
  const imports = `import { ${
    outputTarget.includePolyfills ? `${APPLY_POLYFILLS}, ` : ''
...
function getComponentModelInformation(component: ComponentCompilerMeta) {
  ...
  const modelEvent = events.find((event) => event.name.startsWith(modelProperty.name));
...
function getComponentProperties(properties: any[]): string {
  return properties
    ...
    .map((prop) =>
      prop.isModel
        ? 'model: "<", \n\t\tonModelChange: "&"'
        : `${prop.name}: '${prop.type === 'string' ? '@' : '<'}${prop.optional ? '?' : ''}'`,
    )
    .reduce((bindings, binding) => `${bindings}${binding},\n\t\t`, '');
}
...
function getComponentMethods(methods: any[]): string {
  return methods
    ...
    .reduce((allMethods, method) => `${allMethods}${method},\n\t\t`, '');
}
 ...
 * @return            AngularJS bindings to write
 */
function getComponentEventMethods(componentEvents: any[], componentModelInformation: any): string {
 ...
 */
function getComponentTemplate(componentTagName: string, properties: any[]): string {
  const attributes = properties
    .filter((prop) => BASIC_TYPES.includes(prop.type))
    .sort((prop) => (prop.isModel ? -1 : 0))
    .reduce(
      (attrs, prop) =>
        `${attrs} ${!prop.isModel ? 'ng-attr-' : ''}${prop.attribute}="{{$ctrl.${
          prop.isModel ? 'model' : `${prop.name} || undefined`
        }}}"`,
      '',
    );
 ...
 */
function parseProperties(componentProperties: any[], componentModelInformation: any): any[] {
...
function setComplexItems(properties: any[]): string {
  return properties
    ...
    .reduce((complexBindings, prop) => `${complexBindings}${prop};\n\t\t\t`, '');
}
 ...
 * @return                           Statement to write in the $onInit controller hook.
 */
function setEvents(componentEvents: any[], componentModelInformation: any): string {
  return componentEvents
    .map(
      (event) => `
...
function setPublicMethods(componentMethods: any[]): string {
  return componentMethods
    .map((method) => {
      const parameters = extractParametersFromMethodSignature(method.complexType.signature);
      return `this.${method.name} = (${parameters.join(', ')}) => this.webComponent.${
        method.name
      }(${parameters.join(', ')});`;
    })
    .reduce((methods, method) => `${methods}\n\t\t\t${method}`, '');
}
...
function setOnChangesHook(componentProperties: any[]) {
  const complexProperties = componentProperties.filter((prop) => !BASIC_TYPES.includes(prop.type));
...
function writeComponent(
  ...
  const componentName = `${getComponentTagPrefix(outputTarget)}${convertTagToPascalCase(
    component.tagName,
  )}`;
...
export function generateProxies(
  ...
  return componentsToBuild
    .map((component: ComponentCompilerMeta) => writeComponent(component, outputTarget))
    .reduce((acc, formattedComponent) => `${acc}${formattedComponent}\n`, '');
}
...
export default {
  ${filteredComponents
    .map(
      (comp) =>
        `${getComponentTagPrefix(outputTarget)}${convertTagToPascalCase(comp.tagName)}Module`,
    )
    .join(', ')}
}
  `.replace(/^\s*\n/gm, ''),
